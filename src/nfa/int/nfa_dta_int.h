...
/* DTA control block */
typedef struct {
    uint32_t                dta_flags;      /* dta_flags must be first item in structure (statically intialized to 0 on startup) */
    ...
    uint8_t                 cur_protocol_rank;  /* perference ranking of currently discovered protocol */
    ...
    bool                    llcp_co_more_to_read;    /* TRUE if there is more to read in llcp recieve window*/
...
#define NFA_DTA_LLCP_FLAGS_CO_OUT_CONNECTED     0x02    /* established outbound on connection-oriented  */
    ...
    uint8_t                 llcp_sdp_tid_co;        /* SDP transaction ID for outbound connection-oriented */
